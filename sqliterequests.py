...
class Insert(Request):
    ...
    def __init__(self, table: Table, args: list[str], values: list) -> None:
        args = ', '.join(args)
...
class Select(Request):
    ...
    def __init__(self, table: Table, args: list[str], all = False) -> None:
        if all:
            args = '*'
        else:
            args = ', '.join(args)
...
class Update(Request):
    ...
    def __init__(self, table: Table, args: list[str], values: list) -> None:
        super().__init__()
        args = ', '.join([f"{args[i]} = '{values[i]}'" if type(values[i]) == str else f"{args[i]} = {values[i]}" for i in range(len(args))])
        self.cmd = f'''UPDATE {table.name} SET {args}'''
